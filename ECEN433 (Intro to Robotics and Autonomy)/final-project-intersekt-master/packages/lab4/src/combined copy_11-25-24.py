...
class CombNode:
	def __init__(self):
		...
		# self.redprojectionTopic = f"/{self.botName}/line_detector_node/red_segment_list"	
	...
	def callback(self, data):
		...
		if(self.redLines is not None):
			for i in range(len(self.redLines)):
				...
				redSegmentList.segments.append(thisSeg)
		
		if(whiteYellowSegmentList.segments is not None):
			self.pub_ground.publish(whiteYellowSegmentList)
		if(redSegmentList.segments is not None):
			self.pub_red.publish(redSegmentList)
		return
