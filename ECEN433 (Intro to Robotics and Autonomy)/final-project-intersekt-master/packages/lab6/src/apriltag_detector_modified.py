...
class AprilTagDetectorModified(DTROS):
    def __init__(self):
        super(AprilTagDetectorModified, self).__init__(
        ...
        self._img_sub = rospy.Subscriber(
            f"/{self.botName}/camera_node/image/compressed", CompressedImage, self._img_cb, queue_size=1, buff_size="20MB"
        )
        ...
        self._img_pub = rospy.Publisher(
            "~detections/image/compressed/mod",
            CompressedImage,
            queue_size=1,
    ...
    def _img_cb(self, msg):
        ...
        for i in range(self.ndetectors):
            if self._tasks[i] is None or self._tasks[i].done():
                ...
                ### Here is the edits made, crop image for just top left
                cv_img = self.bridge.imgmsg_to_cv2(msg, "bgr8")
                height, width, _ = cv_img.shape
                cropped_img = cv_img[height//2:, width//2:]
                resized_img = cv2.resize(cropped_img, (cv_img.shape[1], cv_img.shape[0]))
                msg = self.bridge.cv2_to_imgmsg(resized_img, "bgr8")
