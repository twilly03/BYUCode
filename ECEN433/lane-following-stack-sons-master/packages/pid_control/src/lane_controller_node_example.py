...
if __name__ == "__main__":
    ...
    def error_calculate(self, error, kp, ki, kd, error_sum, last_error, last_time, last_control_output):
        current_time = rospy.get_time()
        delta_time = current_time - last_time if last_time else 0.01

        # Apply error thresholding (your existing code)
        if np.abs(error) > self.params["~error_thres"]:
            self.log("error too large, thresholding it!", "error")
            error = np.sign(error) * self.params["~error_thres"]

        # Proportional term
        p = kp * error

        # Integral term with clamping
        if self.min_control_velocity < last_control_output < self.max_control_velocity:
            error_sum += error * delta_time
        error_sum = max(min(error_sum, self.max_error_sum), -self.max_error_sum)
        i = ki * error_sum
        ...
        # Limit control input
        control = max(min(control, self.max_control_velocity), self.min_control_velocity)

        return control, error_sum, error, current_time