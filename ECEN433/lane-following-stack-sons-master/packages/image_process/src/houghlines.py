...
class HoughLines:
    ...
    def output_image_with_lines(self, original_image, lines):
    ...
    def on_image_cropped(self, image_cropped):
        ...
        image_cropped_canny = self.bridge.cv2_to_imgmsg(self.image_cropped_canny_cv, "mono8")
        self.image_edges_pub.publish(image_cropped_canny)
    ...
    def on_image_white(self, image_white):
        ...
        image_white_anded = cv2.bitwise_and(self.image_cropped_canny_cv, image_white_cv)
        image_white_lines = cv2.HoughLinesP(image_white_anded, rho, theta, threshold, None, min_line_length, max_line_gap)
        image_white_hough = self.output_image_with_lines(self.image_cropped_cv, image_white_lines)
        image_white_hough_output = self.bridge.cv2_to_imgmsg(image_white_hough, "bgr8")
        self.image_lines_white_pub.publish(image_white_hough_output)
    ...
    def on_image_yellow(self, image_yellow):
        ...
        image_yellow_anded = cv2.bitwise_and(self.image_cropped_canny_cv, image_yellow_cv)
        image_yellow_lines = cv2.HoughLinesP(image_yellow_anded, rho, theta, threshold, None, min_line_length, max_line_gap)
        image_yellow_hough = self.output_image_with_lines(self.image_cropped_cv, image_yellow_lines)
        image_yellow_hough_output = self.bridge.cv2_to_imgmsg(image_yellow_hough, "bgr8")
        self.image_lines_yellow_pub.publish(image_yellow_hough_output)
